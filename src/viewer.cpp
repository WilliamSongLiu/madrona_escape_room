...
int main(int argc, char *argv[])
{
    ...
    render_asset_paths[(size_t)SimObject::Button] =
        (std::filesystem::path(DATA_DIR) / "cube_render.obj").string();
    render_asset_paths[(size_t)SimObject::Apple] =
        (std::filesystem::path(DATA_DIR) / "apple_render.obj").string();
    ...
    render_assets->objects[(CountT)SimObject::Apple].meshes[0].materialIDX = 6;
